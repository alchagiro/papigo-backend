...
const db = {
  run: (sql, params = [], callback) => {
    const convertedSql = toPgSql(toPgParams(sql));
    pool.query(convertedSql, params)
      .then((result) => {
        if (callback) callback.call({ changes: result.rowCount }, null, handleResult(result, "run"));
      })
      .catch((err) => {
  ...
  get: (sql, params = [], callback) => {
    ...
    pool.query(convertedSql, params)
      .then((result) => {
        if (callback) callback(null, handleResult(result, "get"));
      })
      .catch((err) => {
        if (callback) callback(err);
      });
  ...
  all: (sql, params = [], callback) => {
    ...
    pool.query(convertedSql, params)
      .then((result) => {
        if (callback) callback(null, handleResult(result, "all"));
      })
      .catch((err) => {
        if (callback) callback(err);
      });
...
const initDb = async () => {
  const schema = `
    ...
    CREATE TABLE IF NOT EXISTS platform_debts (
      ...
      platform_percentage DOUBLE PRECISION DEFAULT 25,
